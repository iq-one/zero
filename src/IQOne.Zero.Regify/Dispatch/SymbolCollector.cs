...
internal static class SymbolCollector
{
    ...
    public static HandlerCandidate DescribeHandler(INamedTypeSymbol type, SyntaxNode node)
    {
        ...
        foreach (var attribute in type.GetAttributes())
            attributes.Add(new AttributeUsage(
                ...
                new EquatableArray<string>([.. attribute.ConstructorArguments
                    .Select(a => a.Value as string ?? string.Empty)])));

        var interfaces = ImmutableArray.CreateBuilder<InterfaceUsage>();

        foreach (var @interface in type.AllInterfaces)
            interfaces.Add(new InterfaceUsage(
                Open(@interface),
                new EquatableArray<string>([.. @interface.TypeArguments.Select(t => t.ToDisplayString(Full))])));

        // Base chain of the request type; the derivation check happens during emission.
        var requestChain = ImmutableArray.CreateBuilder<string>();

        var handlerInterface = type.AllInterfaces.FirstOrDefault(i => i.TypeArguments.Length == 2);

        if (handlerInterface?.TypeArguments.FirstOrDefault() is { } request)
            for (var current = request; current is not null; current = current.BaseType)
                requestChain.Add(current.ToDisplayString());
        ...
        return new HandlerCandidate(
            ...
            new EquatableArray<InterfaceUsage>(interfaces.ToImmutable()),
            new EquatableArray<string>(requestChain.ToImmutable()),
