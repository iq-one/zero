---
id: zero.generators.read-the-diagnostic
title: The build error states the fix
package: IQOne.Zero.Generators
applies-to: ["**/*.cs"]
---
...
| --- | --- | --- |
| ZERO006 | A type implements two lifetime markers | Keep the one matching how it is used |
| ZERO007 | No interface to register the class under | Add `I{ClassName}`, or state `[ServiceTypes(typeof(...))]` |
| ZERO008 | A lifetime marker sits on an abstract or generic type | Move it to the concrete class |
| ZERO009 | A singleton takes a scoped dependency | Take `IServiceScopeFactory`, or stop being a singleton |
| ZERO010 | Two implementations register as the same service type | Separate them with a keyed `[ServiceTypes]` |

ZERO006 to ZERO009 are errors and stop the build. ZERO010 is a warning: the container keeps
